...
$(function () {
	...
	var formEditor = UE.getEditor('formEditor', {
		...
		iframeCssUrl: "/smartPlatform/js/bootstrap-3.3.7/css/bootstrap.min.css" //引入自身 css使编辑器兼容你网站css
		//更多其他参数，请参考ueditor.config.js中的配置项
	});
	formEditor.ready(function(){
		//重新设置 edui-editor-iframeholder元素的宽度
		$(".edui-editor-iframeholder").width($(".edui-editor").width());
		var oraHtml = $("#oraHtml").val();
		if(oraHtml){
			oraHtml = decodeHtml(oraHtml);
		}
		formEditor.setContent(oraHtml);
	});
	$(".btn").bind("click",function(){
		var formeditorContent = formEditor.getContent();
		var aaa=formDesign.parse_form(formeditorContent,"");
			formDesign.exec($(this).attr("id"))
	})
...
//			
			...
			fnCheckForm: function(type) {
				...
				if(formEditor.hasContents()) {
					...
					var formeditorContent = formEditor.getContent();
					//解析表单设计器控件
					var parse_form = this.parse_form(formeditorContent, fields);
					//alert(parse_form);
					//异步提交数据
					var _param = $("#formDesignForm").serializeObject();
//					_param['oraHtml'] = encodeHtml(formeditorContent);
